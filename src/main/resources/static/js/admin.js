...
const AdminModule = {
    ...
    getAdminUser() {
        try {
            const raw = sessionStorage.getItem('pawlife_admin_user');
    ...
    checkAdminPermission() {
        if (!this.isAdmin()) {
            if (typeof App !== 'undefined' && App.showToast) {
                App.showToast('Access Denied: Only system Administrators are authorized to perform this administrative action.', 'danger');
            } else {
                alert('Access Denied: Only system Administrators are authorized to perform this administrative action.');
            }
            return false;
        }
        return true;
    ...
    getAuthHeaders() {
        const u = this.getAdminUser();
        return {
            'Content-Type': 'application/json',
            'x-user-role': u && u.role ? u.role : ''
        };
    },
    ...
    renderStaffTable() {
        ...
        tbody.innerHTML = staffMembers.map(u => {
            ...
            const staffId = u.staff_id || `STF-${String(u.user_id).padStart(3, '0')}`;
            ...
            return `
                <tr class="hover:bg-amber-50/50 text-xs transition border-b border-slate-100">
                    <td class="p-3 font-mono font-bold">
                        <span class="px-2.5 py-1 bg-purple-100 text-purple-900 border border-purple-300 rounded-lg text-xs font-black">
                            ${staffId}
                        </span>
                    </td>
                    <td class="p-3">
                    ...
                    <td class="p-3 text-right whitespace-nowrap">
                        <div class="flex items-center justify-end space-x-1.5">
                            <button onclick="AdminModule.openChangePasswordModal(${u.user_id}, '${u.first_name} ${u.last_name}', '${staffId}')" title="Change Password"
                                class="px-2.5 py-1.5 bg-amber-100 hover:bg-amber-200 text-slate-900 rounded-lg border-2 border-slate-900 shadow-[2px_2px_0px_#000] font-bold text-xs transition flex items-center space-x-1 cursor-pointer">
                            ...
                            ` : `
                                <button onclick="AdminModule.toggleUserStatus(${u.user_id}, '${u.status === 'Active' ? 'Inactive' : 'Active'}')"
                                    ...
                                    class="px-2.5 py-1.5 bg-white hover:bg-slate-100 text-slate-800 rounded-lg border-2 border-slate-900 shadow-[2px_2px_0px_#000] font-bold text-xs transition cursor-pointer">
                                ...
                                <button onclick="AdminModule.deleteStaff(${u.user_id}, '${u.first_name} ${u.last_name}', '${u.role}')" title="Delete Staff Account"
                                    class="px-2.5 py-1.5 bg-rose-50 hover:bg-rose-100 text-rose-700 rounded-lg border-2 border-rose-300 font-bold text-xs transition cursor-pointer">
    ...
    renderAdminUsersTable() {
        ...
        tbody.innerHTML = this.cachedUsers.map(u => {
            ...
            const staffId = u.staff_id || `STF-${String(u.user_id).padStart(3, '0')}`;
            return `
                <tr class="hover:bg-slate-50 text-xs transition border-b border-slate-100">
                    <td class="p-3 font-mono font-bold">
                        <span class="px-2 py-0.5 bg-purple-100 text-purple-900 border border-purple-300 rounded font-black text-xs">
                            ${staffId}
                        </span>
                    </td>
                    ...
                    <td class="p-3 text-right">
                        <div class="flex items-center justify-end space-x-1.5">
                            <button onclick="AdminModule.openChangePasswordModal(${u.user_id}, '${u.first_name} ${u.last_name}', '${staffId}')" title="Change Password"
                                class="px-2 py-1 text-xs border rounded bg-amber-50 hover:bg-amber-100 text-slate-800 font-bold">
                                <i class="fas fa-key mr-1 text-amber-600"></i>Password
                            </button>
                            <button onclick="AdminModule.openEditStaffModal(${u.user_id})" class="px-2 py-1 text-xs border rounded hover:bg-slate-100 text-slate-700 font-bold">
                            ...
                            ` : `
                                <button onclick="AdminModule.toggleUserStatus(${u.user_id}, '${u.status === 'Active' ? 'Inactive' : 'Active'}')"
                                    class="px-2 py-1 text-xs border rounded hover:bg-slate-100 text-slate-700 font-bold">
                                ...
                                <button onclick="AdminModule.deleteStaff(${u.user_id}, '${u.first_name} ${u.last_name}', '${u.role}')"
                                    class="px-2 py-1 text-xs border rounded hover:bg-red-50 text-red-600 font-bold">
                                    <i class="fas fa-trash-alt"></i>
                                </button>
                            `}
                        </div>
    ...
    openChangePasswordModal(userId, name, staffId) {
        if (!this.checkAdminPermission()) return;
        document.getElementById('change-pwd-user-id').value = userId;
        const nameEl = document.getElementById('change-pwd-staff-name');
        const idEl = document.getElementById('change-pwd-staff-id');
        if (nameEl) nameEl.textContent = name;
        if (idEl) idEl.textContent = `Staff ID: ${staffId}`;
    ...
    async submitChangePassword() {
        ...
        if (!newPwd || !confirmPwd) {
            App.showToast('Please enter and confirm the new password.', 'warning');
            return;
        }

        if (newPwd !== confirmPwd) {
            App.showToast('Passwords do not match. Please re-enter carefully.', 'danger');
            return;
        }
        ...
        try {
            ...
            const data = await res.json();
            if (!data.success) {
                App.showToast(data.error || 'Failed to change password.', 'danger');
                return;
            }

            App.showToast(data.message || 'Staff password updated successfully!', 'success');
