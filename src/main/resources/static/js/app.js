...
const App = {
    ...
    async init() {
        ...
        if (urlParams.get('auth') === 'admin' || urlParams.get('login') === 'admin') {
            ...
            if (urlParams.get('error') === 'admin_only') {
                const alertBox = document.getElementById('home-auth-error-alert');
                if (alertBox) {
                    alertBox.textContent = 'Access Restricted: Only system Administrators are authorized to access the Admin Management Dashboard.';
        ...
        await GroomingModule.initLiveTracker();
    ...
    logoutAdmin() {
        ...
        localStorage.removeItem('pawlife_admin_user');
        this.showToast('Staff / Administrator signed out.', 'info');
    },
    ...
    openCustomerNutrition() {
        this.showToast('Nutrition & Wellness Consultation: Personalized dietary plans & clinical nutrition available.', 'info');
    },
