...
const AppointmentsModule = {
    ...
    async loadPetsDropdown() {
        ...
        if (typeof CustomerPortal !== 'undefined' && CustomerPortal.currentUser) {
            const u = CustomerPortal.currentUser;
            const ident = u.phone_number || u.email;
            if (ident) {
                const input = document.getElementById('existing-owner-identifier');
                if (input && !input.value) input.value = ident;
                await this.searchMyPets();
            }
        }
    },
    ...
    async searchMyPets() {
        ...
        const errorDiv = document.getElementById('existing-pet-error');
        const select = document.getElementById('book-pet-select');
        const wrapper = document.getElementById('existing-pet-select-wrapper');
        const btn = document.getElementById('btn-find-pets');
        
        if (!identifier) {
            if (errorDiv) {
                errorDiv.textContent = 'Please enter your registered phone number or email address.';
        ...
        if (btn) {
            btn.disabled = true;
            btn.innerHTML = '<i class="fas fa-spinner fa-spin mr-1.5"></i>Searching...';
        }

        try {
            const res = await fetch(`/api/customer/search-pets?identifier=${encodeURIComponent(identifier)}`);
            const data = await res.json();
            
            if (!data.success || !data.pets || data.pets.length === 0) {
                if (errorDiv) {
                    errorDiv.textContent = data.error || 'No pets found for this phone or email. Please check your input or switch to "+ New Customer".';
                    errorDiv.classList.remove('hidden');
                }
                if (wrapper) wrapper.classList.add('hidden');
                return;
            ...
            if (select) {
                select.innerHTML = data.pets.map(p => `
                    <option value="${p.pet_id}">${p.pet_name} (${p.species} • ${p.breed})</option>
                `).join('');
            }
            if (wrapper) {
            ...
            if (errorDiv) {
                errorDiv.textContent = 'Connection error finding pets. Please try again.';
                errorDiv.classList.remove('hidden');
            }
            if (wrapper) wrapper.classList.add('hidden');
            ...
            if (btn) {
                btn.disabled = false;
                btn.innerHTML = '<i class="fas fa-search mr-1.5"></i>Find Pets';
            }
        }
    },
    ...
    setCustomerMode(mode) {
        ...
        if (mode === 'existing') {
            ...
            // Autofill details if logged in
            if (typeof CustomerPortal !== 'undefined' && CustomerPortal.currentUser) {
                const u = CustomerPortal.currentUser;
                const nameInp = document.getElementById('new-owner-name');
                const phoneInp = document.getElementById('new-owner-phone');
                const emailInp = document.getElementById('new-owner-email');
                if (nameInp && !nameInp.value) nameInp.value = u.first_name + ' ' + u.last_name;
                if (phoneInp && !phoneInp.value) phoneInp.value = u.phone_number || '';
    ...
    async confirmBooking() {
        ...
        if (this.customerMode === 'new') {
            ...
            if (!petId) {
                App.showToast('Please find and select your registered pet before confirming.', 'warning');
                document.getElementById('existing-owner-identifier')?.focus();
                return;
            }
    ...
    renderAppointmentsTable(appointments) {
        ...
        tbody.innerHTML = appointments.map(a => `
            <tr class="hover:bg-amber-50/50 text-xs border-b border-slate-200">
                ...
                <td class="p-3">
                    <span class="badge-tag ${
                        (a.status === 'Pending' || a.status === 'Booked') ? 'bg-amber-100 text-amber-900 border border-amber-300 font-bold' :
                        ...
                        <i class="fas ${
                            (a.status === 'Pending' || a.status === 'Booked') ? 'fa-clock text-amber-600' :
                        ...
                        } mr-1"></i>${(a.status === 'Booked' || a.status === 'Pending') ? 'Pending' : a.status}
                    </span>
                </td>
                <td class="p-3 text-right space-x-1 whitespace-nowrap">
                    ...
                    ` : a.status === 'Confirmed' ? `
                        ${a.service_type === 'Grooming' ? `
                            <button onclick="AppointmentsModule.checkInToGrooming(${a.appointment_id}, ${a.pet_id}, '${(a.pet_name || '').replace(/'/g, "\\'")}')"
                                title="Pet arrived: Check-in to Grooming Station Queue"
                                class="px-3 py-1.5 text-[11px] bg-purple-600 hover:bg-purple-700 text-white rounded-lg font-black shadow-sm transition cursor-pointer">
                                <i class="fas fa-cut mr-1"></i>Check-in Grooming
                            </button>
                        ` : `
                    ...
                    ` : `
                        <span class="inline-flex items-center text-xs text-rose-600 font-mono font-bold">
    ...
    async checkInToGrooming(appointmentId, petId, petName) {
        if (!confirm(`Check-in ${petName} into Grooming Salon Queue?`)) return;
        try {
            const res = await fetch('/api/grooming/checkin', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    ...
                    observations: 'Arrived for scheduled grooming visit'
                })
            });
            const data = await res.json();
            if (data.success) {
                App.showToast(data.message || 'Pet checked in to grooming salon!', 'success');
                await this.loadAppointments();
                if (typeof GroomingModule !== 'undefined') {
                    if (typeof GroomingModule.loadQueue === 'function') GroomingModule.loadQueue();
                    if (typeof GroomingModule.initLiveTracker === 'function') GroomingModule.initLiveTracker();
                }
            } else {
                App.showToast(data.error || 'Failed to check-in pet.', 'danger');
            }
        } catch (err) {
            App.showToast('Error: ' + err.message, 'danger');
        }
