...
const CustomerPortal = {
    ...
    async submitLogin(event) {
        ...
        try {
            ...
            if (typeof GroomingModule !== 'undefined' && typeof GroomingModule.initLiveTracker === 'function') {
                GroomingModule.initLiveTracker();
            }
    ...
    async loadPortalData() {
        ...
        if (!this.currentUser) {
            try {
                const raw = localStorage.getItem('pawlife_customer_user');
                if (raw) this.currentUser = JSON.parse(raw);
            } catch (e) {}
        }

        try {
            const uid = this.currentUser ? this.currentUser.user_id : '';
            const oid = this.currentUser ? (this.currentUser.owner_id || '') : '';
            const url = `/api/customer/portal?user_id=${encodeURIComponent(uid)}&owner_id=${encodeURIComponent(oid)}`;
            const res = await fetch(url);
            const data = await res.json();

            if (!data.success) throw new Error(data.error || 'Unable to retrieve pet records.');
            ...
            container.innerHTML = `
                <div class="p-8 text-center text-rose-600 bg-rose-50 rounded-2xl border-2 border-rose-300">
                    ...
                    <div class="font-bold mb-3">Failed to load pet data: ${err.message}</div>
                    <button onclick="CustomerPortal.loadPortalData()" class="px-4 py-2 bg-rose-600 hover:bg-rose-700 text-white rounded-xl text-xs font-bold transition shadow-sm cursor-pointer">
                        <i class="fas fa-redo mr-1.5"></i>Retry Loading
                    </button>
                </div>
            `;
        }
    ...
    renderPortal() {
        ...
        const owner = this.portalData.owner || {};
        const pets = this.portalData.pets || [];
        const appointments = this.portalData.appointments || [];
        const consultations = this.portalData.consultations || [];
        const vaccinations = this.portalData.vaccinations || [];
        const grooming = this.portalData.grooming || [];
        const notifications = this.portalData.notifications || [];

        const fName = owner.first_name || (this.currentUser ? this.currentUser.first_name : 'Pet');
        const lName = owner.last_name || (this.currentUser ? this.currentUser.last_name : 'Parent');
        ...
        container.innerHTML = `
            ...
            <div class="bg-gradient-to-r from-amber-50 to-orange-50 p-6 rounded-2xl border-2 border-slate-900 mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div class="flex items-center space-x-3.5">
                    ...
                    <div>
                        <div class="flex items-center space-x-2">
                            <h3 class="text-xl font-black text-slate-900 font-chunky">${fName} ${lName}'s Pet Family</h3>
                        ...
                        <div class="text-xs text-slate-600 font-medium">
                            <span>📞 ${owner.phone_number || (this.currentUser ? this.currentUser.phone_number : 'N/A')}</span> • <span>✉️ ${owner.email || (this.currentUser ? this.currentUser.email : '')}</span> • <span class="font-bold text-[#FF5A27]">${pets.length} Registered Pet(s)</span>
                        </div>
                    </div>
                </div>
                                ...
                                <div>
                                    <div class="flex items-center space-x-2">
                                        ...
                                        <span class="px-2 py-0.5 rounded-full text-[10px] font-black ${
                                            ...
                                            (a.status === 'Pending' || a.status === 'Booked') ? 'bg-amber-100 text-amber-800 border border-amber-300' :
                                            a.status === 'Cancelled' ? 'bg-rose-100 text-rose-800' :
                                            'bg-slate-100 text-slate-800'
                                        }">${a.status === 'Booked' ? 'Booked (Pending)' : a.status}</span>
                                    </div>
