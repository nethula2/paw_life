...
const PetEhrModule = {
    ...
    async searchPublicPets(searchVal = '') {
        ...
        try {
            ...
            container.innerHTML = data.pets.map(p => {
                const ownerDisplayName = (p.owner_name && p.owner_name.trim()) ? p.owner_name.trim() : 'Registered Client';
                let maskedPhone = 'Contact on File (Clinic Verified)';
                if (p.owner_phone && p.owner_phone.trim()) {
                    const raw = p.owner_phone.trim();
                    if (raw.length >= 7) {
                        maskedPhone = raw.substring(0, 3) + ' ••• ••• ' + raw.substring(raw.length - 2);
                    } else if (raw.length > 2) {
                        maskedPhone = raw.substring(0, 2) + ' ••• ' + raw.substring(raw.length - 1);
                    } else {
                        maskedPhone = '••• ••• (Private)';
                    }
                }
                    ...
                    <div>
                        ...
                        <div class="bg-amber-50/70 p-3 rounded-xl border border-amber-200/70 text-xs space-y-1 mb-4">
                            ...
                            <div class="flex justify-between">
                                ...
                                <span class="font-bold text-slate-800">${ownerDisplayName}</span>
                            </div>
                            <div class="flex justify-between items-center">
                                <span class="text-slate-500">Contact:</span>
                                <span class="font-mono text-slate-700 font-medium flex items-center space-x-1">
                                    <span>${maskedPhone}</span>
                                    <span class="text-[9px] bg-amber-100 text-amber-800 px-1.5 py-0.2 rounded border border-amber-300 font-bold">Privacy</span>
                                </span>
                            </div>
                ...
                <div class="bg-white rounded-2xl border-2 border-slate-900 shadow-[4px_4px_0px_#1E1613] overflow-hidden hover:shadow-[6px_6px_0px_#1E1613] transition">
                    ...
                    <div class="p-5">
                        ...
                        <div class="flex space-x-2">
                            ...
                            <button onclick="PetEhrModule.deletePet(${p.pet_id}, '${p.pet_name}')"
                                class="px-3 py-2 bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-300 rounded-xl text-xs font-extrabold" title="Delete Pet">
    ...
    async submitPetRegistration() {
        try {
            ...
            const ownerPassword = (document.getElementById('reg-owner-password')?.value || '').trim();
            ...
            // Clear input fields
            ['reg-pet-name', 'reg-breed', 'reg-microchip', 'reg-owner-phone', 'reg-owner-email', 'reg-owner-fname', 'reg-owner-lname', 'reg-owner-password', 'reg-dob', 'reg-allergies',
    ...
    async deletePet(petId, petName) {
        if (!confirm(`Are you sure you want to permanently delete the pet profile for ${petName}? This action cannot be undone.`)) {
            return;
        }
        ...
        try {
            ...
            if (!data.success) {
                if (toastFn) toastFn(data.error || 'Failed to delete pet.', 'danger');
                return;
            }
            
            if (toastFn) toastFn(data.message || `Deleted pet ${petName}.`, 'success');
            ...
            console.error(err);
            const toastFn = (typeof App !== 'undefined' && App.showToast) || (typeof DashboardApp !== 'undefined' && DashboardApp.showToast);
            if (toastFn) toastFn('Error deleting pet.', 'danger');
        }
    }
};
